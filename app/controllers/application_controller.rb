class ApplicationController < ActionController::Base
  ...
  def sbrf_action
    if Rails.env.production?
      ...
      @sbrf_client = SBRF::Acquiring::Client.new(username: Rails.application.credentials.dig(:sbrf_test, :username),
        password: Rails.application.credentials.dig(:sbrf_test, :password), test: true)
    end
  end
